<?php
	...
	if (isset($_POST["nomeAttivita"]) and 
		...
		$nameBusiness = $_POST["nomeAttivita"];
		$P_IVA = $_POST["P_IVA"];
		$nomeRef = $_POST["nomeReferente"];
		$cognomeRef = $_POST["cognomeReferente"];
		...
		if($pwd == $confermaPwd) {
			...
			if (mysqli_num_rows($result) <= 0) {
				$stmt = $conn->prepare('INSERT INTO fornitori(nomeRistorante, P_IVA, nomeReferente, cognomeReferente, telefono, indirizzoMaps, orarioApertura, orarioChiusura, email, password) VALUES(?,?,?,?,?,?,?,?)');
				$stmt->bind_param("ssssssss", $P_IVA, $nameBusiness, $descrizione, $maps, $email, $pwd, $orApertura, $orChiusura);
				$stmt->execute();
				$stmt->close();
